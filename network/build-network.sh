...
build_docker() {
   ...
   DOCKER_BUILDKIT=1 docker build $flags --build-arg COMMIT_HASH=$branch --tag ibcbridgedemo:$title -f template/$title/Dockerfile.$title . | true
...
while getopts ajr flag; do
   case "${flag}" in
      ...
      j) install_binary juno https://github.com/CosmosContracts/juno.git v12.0.0
         ...
         ;;
      r) install_binary relayer https://github.com/cosmos/relayer.git andrew/client_icq_stride_v3
